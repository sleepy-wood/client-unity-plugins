...
class SleepDetectorInput: MLFeatureProvider {
    ...
    convenience init(
        accs: MLShapedArray<Float>,
        hvs: MLShapedArray<Float>,
        hds: MLShapedArray<Float>
    ) {
...
class SleepDetectorOutput: MLFeatureProvider {
    ...
    var out_: MLMultiArray {
        provider.featureValue(for: "out")!.multiArrayValue!
    }

    /// out as multidimensional array of floats
    @available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *)
    var out_ShapedArray: MLShapedArray<Float> {
        MLShapedArray<Float>(out_)
    }
    ...
    init(out_: MLMultiArray) {
        provider =
            try! MLDictionaryFeatureProvider(dictionary: ["out": MLFeatureValue(multiArray: out_)])
    }
...
class SleepDetector {
    ...
    class func load(
        ...
        completionHandler handler: @escaping (Swift.Result<SleepDetector, Error>) -> Void
    ) {
        load(
            contentsOf: urlOfModelInThisBundle,
            configuration: configuration,
     ...
     */
    @available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *)
    class func load(configuration: MLModelConfiguration = MLModelConfiguration()) async throws
        -> SleepDetector
    {
        try await load(contentsOf: urlOfModelInThisBundle, configuration: configuration)
    }
    ...
    /**
         ...
         - returns: the result of the prediction as SleepDetectorOutput
     */
    func prediction(input: SleepDetectorInput,
                    options: MLPredictionOptions) throws -> SleepDetectorOutput
    {
    ...
    /**
         ...
         - returns: the result of the prediction as SleepDetectorOutput
     */
    func prediction(accs: MLMultiArray, hvs: MLMultiArray,
                    hds: MLMultiArray) throws -> SleepDetectorOutput
    {
